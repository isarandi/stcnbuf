...
We will resize the input video to 480p -- check generic_test_dataset.py if you want to change
this behavior
AMP default on.
...
def main():
    ...
    parser.add_argument('--include_last', help='include last frame as temporary memory?',
                        action='store_true')
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            ...
            for i, frame_idx in enumerate(frames_with_gt):
                ...
                with_bg_msk = torch.cat([
                    ...
                    msk[:, frame_idx]], 0).cuda()

                # We perform propagation from the current frame to the next frame with label
                if i == len(frames_with_gt) - 1:
                    print(f'processor.interact(with_bg_msk, {frame_idx}, {rgb.shape[1]}, {obj_idx})')
                    print(with_bg_msk.shape)
                    processor.interact(with_bg_msk, frame_idx, rgb.shape[1], obj_idx)
                else:
                    print(f'processor.interact(with_bg_msk, {frame_idx}, {frames_with_gt[i + 1] + 1}, {obj_idx})')
            ...
            for f in range(idx_masks.shape[0]):
                if f >= min_idx:
                    ...
                    img_E.save(
                        os.path.join(this_out_path, info['frames'][f][0].replace('.jpg', '.png')))
